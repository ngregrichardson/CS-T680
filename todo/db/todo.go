...
//
// ANSWER: This is a good decision because it prevents any external user of the
//
//	struct from directly accessing the fields. This is important
//	because it prevents the user from directly modifying the fields
//	and potentially corrupting the data. Instead, the user must use
//	the functions provided by the struct to modify the data, which
//	ensures that the data is modified in a safe way.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) GetID(id string) (int, error) {
